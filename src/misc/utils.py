...
def run_simulation_d(simulation_object, instances, processes=1, batch=100, silent=False):
    ...
    while remaining_instances > 0:
        ...
        position_pool, velocity_pool, acceleration_pool,\
            edge_pool,d_position_velocity_pool,d_n_view_pool,\
            d_edge_pool,d_next_position_velocity_pool,\
            d_next_acceleration_pool = zip(*data_pool)
        ...
        d_position_velocity_data_all.extend(d_position_velocity_pool)
        d_n_view_data_all.extend(d_n_view_pool)
        d_edge_data_all.extend(d_edge_pool)

        d_next_position_velocity_data_all.extend(d_next_position_velocity_pool)
        d_next_acceleration_data_all.extend(d_next_acceleration_pool)
